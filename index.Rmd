...
* What makes you want to learn R?
...
Having said this, we advocate specifically the [statistical programming language R](https://www.r-project.org/about.html) and the [RStudio](https://rstudio.com/) code editor.
...
RStudio is a popular and well-supported piece of software for editing and running R code for both beginners and advanced users. It's also free of charge and the company behind it is [a public benefit coprporation](https://blog.rstudio.com/2020/01/29/rstudio-pbc) with a commitment to producing open source software.
...
* Create two new folders -- `data` and `output` -- in your Project folder (we'll be using these later)
* We haven't created the R script files yet, but they'll go in the project folder

To access your RStudio Project in future, navigate to the project folder and double-click your R Project file, which has the .Rproj extension (e.g. `your-project.Rproj`). It will open in the same state that you left it when you last closed it.
...
* Running `my_num` is euivalent to `print(my_num)`
* I'll be using `print()` throughout to be more explicit

</div>
<br>

The real benefit to this is that you don't have to repeat yourself every time you want to use that particular calculation. For example, you can refer to it in new expressions:
...
* What value do you get when you print `calculation`?
...
So we've create objects composed of a single values (`my_num`) and a vector (`dept_names`). You can think of these as being zero-dimensional and one-dimensional, respectively.

The next step would be two dimenions: a table with rows and columns. Tables of data with rows and columns are called 'data frames' in R and are effectively a bunch of vectors of the same length stuck together.
...
add_nums(val_a = 3, val_b = 4)  # use the 
```
</div>
<br>
...
* [{haven}](https://haven.tidyverse.org/) for reading a range of file types, like Stata (.dta) and SPSS (.sav)
...
So we can install all these packages and more at once with:
...
download.file(
  url = "",
  destfile = "data/csps_synth.dta"
)
```

The variables are in the synthetic data set are:

* ResponseID (unique respondent identifier) and OverallDeptCode (the department name)
...
* How do you know that the data has been successfully read?
...
To save time you can use some special `select()` helper functions. For example, you can select a column that `contains()` or `starts_with()` certain strings. This is useful if you have lots of columns that share a simmilarity in their names, like the CSPS questions (e.g. B01, B02, etc, all start with "B").
...
| `%in%` | Not equal to | `4 == 6` returns `TRUE` |
...
```{r filter-result}
select(scs_orgs, OverallDeptCode, Z02)
```
...
* people who responded with 'strongly agree' (`5`) to B01 *and* B02 get a `1`
* and everyone else gets a `0`
...
* There's more than one function called `as_factor()`; how can we be disambiguous?
...
We can use the join functions to merge two data frames together on a common column.
...
* We'll use the `tibble()` function from {dplyr} to build the data frame
* Tibbles are data frames with nice defaults and printing properties; we've seen them already in the outputs from our earlier wrangling with {dplyr}
...
lookup <- tibble(
  ...
  dept_full_name = c("Dept of A", "Ministry for B", "Agency of C")
)
...
Here's what we're doing (gif by [Garrick Aden-Buie](https://www.garrickadenbuie.com/project/tidyexplain/)):
...
We could have a whole separate session on visualising data (maybe we will).

For now, let's look at a simple bar chart of the answers to question B01 using the `ggplot()` function from the {ggplot2} package.
...
* gave it some aesthetics, `aes()`; in this case the x and y variables
* used the column geom, `geom_col()`, to make a bar chart

We can spruce this up a little.
...
But we could also split each department's results into its own 'facet'.
